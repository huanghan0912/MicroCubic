...
void Game2048UIInit(void)
{
    ...
    if (act_obj == game2048_src)
        return;
    lv_obj_clean(act_obj); // 清空此前页面

    //创建屏幕对象
    game2048_src = lv_obj_create(NULL);
    lv_obj_add_style(game2048_src, &default_style, LV_STATE_DEFAULT);

    for (int i = 0; i < SCALE_SIZE * SCALE_SIZE; i++)
    {
        img[i] = lv_img_create(game2048_src);
        lv_img_set_src(img[i], &N0);
        lv_obj_align(img[i], LV_ALIGN_TOP_LEFT, 8 + i % 4 * 58, 8 + i / 4 * 58);
    }
    lv_scr_load(game2048_src);
}
