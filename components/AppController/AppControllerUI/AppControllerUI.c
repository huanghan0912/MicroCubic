...
void DisplayAppScrInit(const void *src_img_path, const char *app_name){
...
void AppControlDisplayScr(const void *src_img, const char *app_name, lv_scr_load_anim_t anim_type, bool force)
{
    // force为是否强制刷新页面 true为强制刷新
    if (true == force)
    {
        DisplayAppScrInit(src_img, app_name);
        return;
    }
